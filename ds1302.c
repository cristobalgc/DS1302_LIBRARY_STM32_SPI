...
/** \file sw_component.c
 *
 * \brief Source code file for SW_COMPONENT
 *
 * Source code file for SW_COMPONENT manager
 *
...
/* Months of a year */
static char const *ds1302_months[]={DS1302_UNKNOWN, DS1302_JAN, DS1302_FEB, DS1302_MAR, DS1302_APR, DS1302_MAY, DS1302_JUN, DS1302_JUL, DS1302_AUG, DS1302_SEP, DS1302_OCT, DS1302_NOV, DS1302_DIC};
/* AM/PM*/
static char const *ds1302_AMPM[]={DS1302_AM, DS1302_PM};
/* Write protect command (disable) */
static uint8_t ds1302_wp[2] = {DS1302_ENABLE, 0x00};
/* Trickle charge command (disable) */
static uint8_t ds1302_trickle[2] = {DS1302_TRICKLE, 0x00};
 ...
 */
static HAL_StatusTypeDef ds1302_write(ds1302_T *ds1302, uint8_t *pData, uint16_t Size, uint32_t Timeout);
...
/**
  ...
  * @param  hspi: SPI Handler.
  * @param  val: the received value though SPI port.
...
static HAL_StatusTypeDef ds1302_write(ds1302_T *ds1302, uint8_t *pData, uint16_t Size, uint32_t Timeout)
{
  ...
  /* Process Locked */
  __HAL_LOCK(ds1302->cfg.spi);

  /* Init tickstart for timeout management*/
  tickstart = HAL_GetTick();
  initial_TxXferCount = Size;

  if (ds1302->cfg.spi->State != HAL_SPI_STATE_READY){
    errorcode = HAL_BUSY;
    goto error;
  }
  ...
  ds1302->cfg.spi->State       = HAL_SPI_STATE_BUSY_TX;
  ds1302->cfg.spi->ErrorCode   = HAL_SPI_ERROR_NONE;
  ds1302->cfg.spi->pTxBuffPtr  = (uint8_t *)pData;
  ds1302->cfg.spi->TxXferSize  = Size;
  ds1302->cfg.spi->TxXferCount = Size;

  /*Init field not used in handle to zero */
  ds1302->cfg.spi->pRxBuffPtr  = (uint8_t *)NULL;
  ds1302->cfg.spi->RxXferSize  = 0U;
  ds1302->cfg.spi->RxXferCount = 0U;
  ds1302->cfg.spi->TxISR       = NULL;
  ds1302->cfg.spi->RxISR       = NULL;

  /* Configure communication direction : 1Line */
  if (ds1302->cfg.spi->Init.Direction == SPI_DIRECTION_1LINE){
    /* Disable SPI Peripheral before set 1Line direction (BIDIOE bit) */
    __HAL_SPI_DISABLE(ds1302->cfg.spi);
    __SPI_DIRECTION_1LINE_TX(ds1302->cfg.spi);
  }
  ...
  {
    if ((ds1302->cfg.spi->Init.Mode == SPI_MODE_SLAVE) || (initial_TxXferCount == 0x01U)){
      *((__IO uint8_t *)&ds1302->cfg.spi->Instance->DR) = (*ds1302->cfg.spi->pTxBuffPtr);
      ds1302->cfg.spi->pTxBuffPtr += sizeof(uint8_t);
      ds1302->cfg.spi->TxXferCount--;
    }
    while (ds1302->cfg.spi->TxXferCount > 0U){
      /* Wait until TXE flag is set to send data */
      if (__HAL_SPI_GET_FLAG(ds1302->cfg.spi, SPI_FLAG_TXE)){
        *((__IO uint8_t *)&ds1302->cfg.spi->Instance->DR) = (*ds1302->cfg.spi->pTxBuffPtr);
        ds1302->cfg.spi->pTxBuffPtr += sizeof(uint8_t);
        ds1302->cfg.spi->TxXferCount--;
      } else {
...
//  }
  ...
  if (ds1302->cfg.spi->ErrorCode != HAL_SPI_ERROR_NONE){
    errorcode = HAL_ERROR;
  }

error:
ds1302->cfg.spi->State = HAL_SPI_STATE_READY;
  /* Process Unlocked */
  __HAL_UNLOCK(ds1302->cfg.spi);
  return errorcode;
}
...
static ds1302_errors_t ds1302_Read1Byte(ds1302_T *ds1302, uint8_t *val)
{
	...
	__disable_irq();
	__HAL_SPI_ENABLE(ds1302->cfg.spi);
	//for(delay=0;delay<110;delay++){__DSB();}
...
ds1302_errors_t DS1302_Write(ds1302_T *ds1302, uint8_t *data, uint8_t size){
	...
	error = ds1302_write(ds1302, data, size, 100);
...
ds1302_errors_t DS1302_Read(ds1302_T *ds1302, uint8_t RegisterAddr, uint8_t *ptr, uint8_t nbytes){
	...
	RegisterAddr|= 0x01;// last bit should be set for read

	HAL_GPIO_WritePin(ds1302->cfg.RstPin.McuPort, ds1302->cfg.RstPin.Pinreset, GPIO_PIN_SET);

	ds1302_write(ds1302, &RegisterAddr, 1, 100);
	/* Check if we need to read one byte or more */
	if(nbytes > 1U){
		error = ds1302_ReadNBytes(ds1302, ptr, nbytes,100);
	}else{
...
ds1302_errors_t DS1302_setTime(ds1302_T *ds1302, uint8_t seconds, uint8_t minutes, uint8_t hours, uint8_t dayofweek,
	...
	if((seconds <= DS1302_SECONDS_MAX) && (minutes <= DS1302_MINUTES_MAX) &&
			...
			(dayofmonth <= DS1302_MONTH_MAX) && (month <= DS1302_MONTHDAY_MAX) &&
		...
		ds1302->data.send.Hour.h24.b.hour_12_24 = 0;// 0 for 24 hour format

		ds1302->data.send.Date.b.Date10 = DS1302_BIN2BCD_H(dayofmonth);
		ds1302->data.send.Date.b.Date = DS1302_BIN2BCD_L(dayofmonth);
		...
		ds1302->data.send.Year.b.Year10 = DS1302_BIN2BCD_H(year-2000);
		ds1302->data.send.Year.b.Year = DS1302_BIN2BCD_L(year-2000);

		ds1302->data.send.WriteProct.b.WP = 0;
	}
...
ds1302_errors_t DS1302_updateDateTime(ds1302_T *ds1302) {
	...
	if(ds1302 != NULL){
		...
		ds1302->data.dateandtime.monthday =  DS1302_BCD2BIN(ds1302->data.received.Month.b.Month10, ds1302->data.received.Month.b.Month);
